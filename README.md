...
**Resposta:** Transfer-Encoding: chunked. Corpo retornado é HTML. Descobri atrvés de análise da resposta da requisiçã
...
```json
{
  ...
  "headers": "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7", 
             ...
             "Upgrade-Insecure-Requests": "1", 
             "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36", 
             "X-Amzn-Trace-Id": "Root=1-69ff857d-2ad379456d081ec850da7b97"
  "origin":  "187.58.19.47"
}
```
...
**Resposta:** O campo origin corresponde ao endereço IP público do cliente, ou seja, o IP pelo qual sua requisição é vista na internet. Ele não é o IP local porque:

  Esses IPs são privados e não são roteáveis na internet.
  A requisição passa por um roteador/NAT (Network Address Translation).
  O NAT substitui o IP local pelo IP público da rede antes de enviar o pacote.
...
|-------------------------------|--------------------------------|------------------------------------------------------------------------------------|
| X-Forwarded-For               | Proxy / balanceador de carga   | Indica o IP original do cliente antes de passar por intermediários                 |
| X-Forwarded-Proto             | Proxy / gateway (ex: CDN, AWS) | Informa se a requisição original foi HTTP ou HTTPS                                 |
| X-Amzn-Trace-Id               | Infraestrutura da AWS          | Usado para rastreamento da requisição dentro da rede da AWS (monitoramento/debug)  |
...
```
custname=Henrique&custtel=%2B5513981608574&custemail=hahollanda%40gmail.com&size=medium&topping=bacon&topping=mushroom&delivery=20%3A00&comments=
```
...
"form": {
  comments=
  custemail=hahollanda@gmail.com
  custname=Henrique
  custtel=+5513981608574
  delivery=20:00
  size=medium
  topping
      bacon
...
**Resposta:** O formato do corpo é application/x-www-form-urlencoded. Nesse formato:

Os dados são enviados como pares chave=valor separados por &.
Espaços são codificados como +.
Caracteres especiais (como acentos) são codificados em percent-encoding (URL encoding), por exemplo:
á → %C3%A1
ç → %C3%A7
...
{
  "args": {}, 
  "data": "{\"protocolo\":\"HTTP\",\"versao\":\"1.1\"}", 
  "files": {}, 
  "form": {}, 
  "headers": {
    "Content-Length": "35", 
    ...
    "Host": "httpbin.org", 
    "User-Agent": "Fiddler", 
    "X-Amzn-Trace-Id": "Root=1-69ff8d4e-1a24cded6158fa9c095ee175"
  }, 
  "json": {
    "protocolo": "HTTP", 
    "versao": "1.1"
